...
with open('../config/binnings.json', 'r') as file:
...
def guarantor_binning(raw_data):
    ...
    for group, guarantors in guarantor_bins.items():
        raw_data.loc[raw_data["guarantor"].isin(guarantors), "guarantor"] = group
    return raw_data

def loan_type_binning(raw_data):
    loan_type_bins = binnings["loan_type_bins"]
    for group, loan_types in loan_type_bins.items():
        raw_data.loc[raw_data["loan_type"].isin(loan_types), "loan_type"] = group
    return raw_data
...
def principal_amount_binning(raw_data):
    ...
    for bin in principal_amount_bins:
        ...
        mask = (raw_data["principal_amount"].astype(float) > lower) & (raw_data["principal_amount"].astype(float) <= upper)
...
def one_hot_encoding(binned_data):
    ...
    for feature, categories in category_dict.items():
        for category in categories:
            binned_data[f"{feature}:{category}"] = (binned_data[feature] == category).astype(int)
